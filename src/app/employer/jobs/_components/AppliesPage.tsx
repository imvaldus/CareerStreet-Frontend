...
export default function AppliesPage({ applyList }: AppliesPageProps) {
  ...
  // Tính toán số trang và danh sách đơn ứng tuyển hiện tại
  const totalPages = Math.ceil((appliesListByEmployerId?.length || 0) / itemsPerPage);
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = appliesListByEmployerId?.slice(indexOfFirstItem, indexOfLastItem) || [];
        ...
        <div className="overflow-x-auto">
          ...
          <div className="flex items-center justify-between px-6 py-4 bg-white border-t border-gray-200">
            <div className="flex items-center text-sm text-gray-700">
              <span>
                Hiển thị {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, appliesListByEmployerId?.length || 0)} trong {appliesListByEmployerId?.length || 0} đơn ứng tuyển
              </span>
            </div>
